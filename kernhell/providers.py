...
def generic_call(provider: str, api_key: str, system_prompt: str, user_prompt: str) -> Optional[str]:
    """Generic AI text generation for any KernHell module (Generator, Analyzer, etc.)."""
    try:
        if provider == "google":
            import google.generativeai as genai
            genai.configure(api_key=api_key)
            ...
            return _clean_response(response.text) if response.text else None
        ...
        log_warning(f"Generic AI call failed ({provider}): {e}")
        return None
    return None
